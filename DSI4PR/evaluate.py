...
class Args:
    expid: str = 'kitti'
    eval_sequence: str = '00'
    threshold_dist: int = 10

args = tyro.cli(Args)

CFG = importlib.import_module(f"CFG.{args.expid}").CFG

model = importlib.import_module(f"model.{CFG.model}").Model(CFG)
get_topk = importlib.import_module(f"model.{CFG.model}").get_topk
...
def main():
    ...
    all_filenames = get_filenames([args.eval_sequence], CFG.data_path)

    if len(args.eval_sequence) == 2:
        translation_poses,_ = get_poses(args.eval_sequence, CFG)
    elif len(args.eval_sequence) == 4:
        translation_poses, indices= get_poses(args.eval_sequence, CFG)
        indices_index = np.array(list(indices.keys()))
        all_filenames = all_filenames[indices_index.astype(int)]

...
def loadCloudFromBinary(file, cols=3):
    ...
    if len(bin_pcd) % 4 != 0:
            print(f"Skipping file {file} due to invalid data length.")
            return None
    points = bin_pcd.reshape(-1, 4)
    #points = bin_pcd[:(bin_pcd.shape[0] // 4) * 4].reshape(-1, 4)
    return points
    
import numpy as np
from PIL import Image

def createRangeImage(points, crop_distance=True, distance_threshold=50):
    if crop_distance:

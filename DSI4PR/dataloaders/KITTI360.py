...
class KITTI360Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if(self.fisheye):
            ...
            image = cv2.imread(f"{self.data_path}/data_2d_raw/2013_05_28_drive_{seq}_sync_image_00/2013_05_28_drive_{seq}_sync/image_00/depth/{instance}.png")

        if image is None:
            print("image is none", f"{self.data_path}/data_2d_raw/2013_05_28_drive_{seq}_sync_image_00/2013_05_28_drive_{seq}_sync/image_00/depth/{instance}.png")

        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        image = self.transforms(image=image)['image']
        item['depth_image'] = torch.tensor(image).permute(2, 0, 1).float()
